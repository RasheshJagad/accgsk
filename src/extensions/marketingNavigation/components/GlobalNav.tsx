...
const GlobalNav: React.FunctionComponent<IGlobalNavProps> = (props: IGlobalNavProps) => {
    ...
    const loadNavigations = async (): Promise<void> => {
        ...
        $(props.domElement).closest(`div[data-sp-placeholder="Top"]>div`).find(`#configinfo`).closest(`div.ms-SPLegacyFabricBlock`).remove();
    ...
    const jQueryLoad = (): void => {
        ...
        $(document).on('mouseenter', `.${styles.dropMenuCustomGridItem}`, function (ctrl) {
            if ($(window).width() as number > 1100) {
                let $this = $(ctrl.currentTarget);
                let sideArrowPosition = $this.position().top;
                let findParent = $this.children('a');
        ...
        $(document).on('mouseleave', `.${styles.dropMenuCustomGridItem}`, function (event) {
            ...
            var sideArrow = $(document).find(`.${styles.sideArrow}`);
            if ($(window).width() as number > 1100) {
    ...
    const embedSearch = (): void => {
        ...
        $('#sbcId').empty();
        $(formDiv).append(addSearchBox(rootSiteUrl));
    }
    ...
    const addConfigMessage = () => {
        if ($(props.domElement).closest(`div[data-sp-placeholder="Top"]`).find('#configinfo').length < 1) {
            $(props.domElement).closest(`div[data-sp-placeholder="Top"]>div`).append(`<div class="ms-SPLegacyFabricBlock"><span id="configinfo">Please configure the 'RootSiteUrl' in <a href="${props.context.pageContext.site.absoluteUrl}/Lists/Configurations">Configurations List</a></span></div>`);
        }
        setTimeHandler();
        setTimeSideHandler();
    ...
    React.useEffect(() => { getSiteTitle(); }, [rootSiteUrl]);
    React.useEffect(() => { getRootSiteUrl(); }, []);
    React.useEffect(() => {
